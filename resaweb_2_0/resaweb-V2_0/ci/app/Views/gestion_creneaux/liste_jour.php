        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid p-0">
                    ...
                    <a href="<?= site_url('crenaux') ?>" class="btn btn-secondary mb-3">
                        ← Retour
                    </a>

                    <h1 class="h3 mb-3"><strong>Réservations du <?= esc($date) ?></strong></h1>

                    <?php if (empty($reservations)): ?>
                        <div class="alert alert-info">Aucune réservation pour l'instant !</div>
                    <?php endif; ?>
                    ...
                    <?php foreach ($grouped as $salle => $liste): ?>
                        <div class="card mb-4">

                            <div class="card-header bg-primary text-white">
                                <strong>Salle <?= esc($salle) ?></strong> — Réservations
                            </div>

                            <ul class="list-group list-group-flush">
                                <?php foreach ($liste as $r): ?>
                                    <li class="list-group-item">
                                        <strong>Heure : </strong><?= esc($r['res_heuredebut']) ?><br>
                                        <strong>Responsable : </strong><?= esc($r['responsable'] ?? 'Non défini') ?><br>
                                        <strong>Participants : </strong><?= esc($r['participants']) ?>
                                        <br>

                                        <?php if ($r['res_datereservation'] < date('Y-m-d') && !empty($r['res_bilanreservation'])): ?>
                                            <strong>Bilan :</strong> <?= esc($r['res_bilanreservation']) ?><br>
                                        <?php endif; ?>
                                    </li>
                                <?php endforeach; ?>
                            </ul>
                    ...
                    <!-- INDISPONIBILITES -->
                    <h2 class="h4 mt-5">Indisponibilités</h2>

                    <?php if (empty($indispos)): ?>
                        <p>Aucune indisponibilité ce jour.</p>
                    <?php else: ?>
                        <?php foreach ($indispos as $ind): ?>
                            <div class="card mb-3">
                                <div class="card-header bg-warning text-dark">
                                    Salle <?= esc($ind['salle']) ?> — Indisponible
                                </div>
                                <div class="card-body">
                                    <p><strong>Début :</strong> <?= esc($ind['ids_datedebut']) ?></p>
                                    <p><strong>Fin :</strong> <?= esc($ind['ids_datefin']) ?></p>
                                    <p><strong>Motif :</strong> <?= esc($ind['motif']) ?></p>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
